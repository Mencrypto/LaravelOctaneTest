        ...
        <li class="flex justify-between gap-4 py-4">
            <div class="flex gap-4">
                <div class="size-8 rounded-full flex items-center justify-center" style="background-color: {{ $question->category->color }};">
                    <x-forum.logoquestion class="h-6 text-white"/>
                </div>
                <div class="flex-auto">
                    <p class="text-sm font-semibold text-gray-900">
                        <!-- false to set relative URL -->
                        <a href="{{ route('questions.show', $question->id, false) }}" class="hover:underline">{{ $question->title}}</a>
                    </p>
